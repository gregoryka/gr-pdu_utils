      ...
      d_wall_clock_time(false)
{
    ...
    this->d_logger->info("starting at time {{{} {}}}",
...
void tags_to_pdu_impl<T>::handle_ctrl_msg(pmt::pmt_t ctrl_msg)
{
    ...
    if (pmt::dict_has_key(ctrl_msg, PMTCONSTSTR__eob_offset())) {
        ...
        this->d_logger->info(
            "command received - set EOB tag offset to {} symbols",
                d_eob_offset);
    }
    ...
    if (pmt::dict_has_key(ctrl_msg, PMTCONSTSTR__eob_alignment())) {
        ...
        if (new_eob_alignment > 0) {
            ...
            this->d_logger->info(
                "command received - set EOB tag alignment to {} symbols",
                    d_eob_alignment);
        } else {
            this->d_logger->error(
                         "command received - illegal value {} for EOB "
    ...
    if (d_triggered) {
        ...
        if ((d_tag_type != NONE) && (d_tag.offset <= last_addr_in_vec)) {
            ...
            if (d_tag_type == EOB) {
                ...
                this->d_logger->error(
                             "SOB tag received during burst {} at offset "
